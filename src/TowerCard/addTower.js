...
const clickTowerCoverHandler = function (e) {
    ...
    towerCards.forEach((towerCard, i) => { // click들 초기화
...
const clickBattlegroundFeildHandler = function (e) {
    ...
    for (let road of cantBuildRoads) { // 지을 수 없는 길인 경우 불가
        if (e.target.dataset.position === road) { return }
    }
    if (!currentBtn || currentBtn === 'wall') { return } // 선택된 포탑이 없으면 불가
    if (currentBtn === 'slow' && slowTowerLimit === false) { // 슬로우 타워는 한번에 하나씩 지을 수 있다.
        slowTowerLimit = true
