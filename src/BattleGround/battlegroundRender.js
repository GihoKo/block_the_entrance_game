...
(function battlegroundRender() {
    for (let i = 0; i < 7; i++) {// 가로 for문 돌리기 (i)
        for (let j = 0; j < 12; j++) {// cell for문 돌리기 (j)
            ...
            entrances.forEach((entrance, i) => { // entrance
                if (div.dataset.position === entrance) {
                    div.style.background = 'url(./img/entrance.png)'
                }
            })
            roads.forEach((road, i) => { // road
...
export { battlegroundFeild, enterEntrance, exitEntrance };